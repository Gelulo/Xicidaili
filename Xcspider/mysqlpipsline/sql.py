...
class Sql():
    ...
    def insert_db_xici(cls,country,ipaddress,port,serveraddr,isanonymous,type,alivetime,verifictiontime):
        sql = 'insert into xici(country,ipaddress,port,serveraddr,isanonymous,type,alivetime,verifictiontime) VALUES (%(country)s,%(ipaddress)s,%(port)s,%(serveraddr)s,%(isanonymous)s,%(type)s,%(alivetime)s,%(verifictiontime)s)'
        value = {
            "country":country,
            "ipaddress":ipaddress,
            "port":port,
